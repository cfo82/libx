...
protected:
	char ReadCharacter(TemporalInfo& info);
	void SkipWhitespaces(TemporalInfo& info);
	void EatComment(TemporalInfo& info);
	void ReadGuidField(TemporalInfo& info, char* field, const uint_t length);
	void ReadGuidSeparator(TemporalInfo& info);
	udword_t HexStringToInt(const char* field, const uint_t length);
