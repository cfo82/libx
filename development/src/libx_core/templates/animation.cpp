...
)
{
	if (!read_cbrace)
	{
		...
		info.error_message += UNICODE_STRING("template 'Animation' cannot be included without reading the final CBRACKET-token!");
		return;
	}
	...
	while (!info.error_flag && continue_flag)
	{
		Token* next_token = info.scanner->ReadToken(info);

		switch (next_token->GetType())
		{
		case Token::TypeName: { // child template
			Template* template_data = read_template_data(next_token, info);
			if (template_data)
			{
				children_.push_back(template_data);
			}
			else
			{
				info.error_flag = true;
		...
		case Token::TypeOBrace: { // reference to another template instance
			Token* reference_name = info.scanner->ReadToken(info);
			if (reference_name->GetType() == Token::TypeName)
			{
				reference_ = static_cast<TokenName*>(reference_name)->GetName();
			}
			...
			{
				info.error_flag = true;
				info.error_message += UNICODE_STRING("Expected closing brace token after reference to template instance.\n");
			}
			break;
		}
