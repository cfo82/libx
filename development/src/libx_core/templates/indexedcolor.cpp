...
Template* IndexedColor::Clone() const
{
	...
	if (index_color_ != NULL)
		clone->index_color_ = static_cast<ColorRGBA*>(index_color_->Clone());
	return clone;
}
...
{
	index_ = get_next_dword(info);
	index_color_ = static_cast<ColorRGBA*>(instanciate_template_by_name(info, UNICODE_STRING("ColorRGBA")));
