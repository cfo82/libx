...
Template* DeclData::Clone() const
{
	...
	{
		clone->elements_.push_back(static_cast<VertexElement*>((*begin)->Clone()));
	}
...
{
	elements_.clear();
	element_count_ = get_next_dword(info);
	for (udword_t i = 0; i < element_count_; ++i)
	{
		VertexElement* current_element = static_cast<VertexElement*>(instanciate_template_by_name(info, UNICODE_STRING("VertexElement")));
