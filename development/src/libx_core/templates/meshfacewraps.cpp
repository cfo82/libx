...
Template* MeshFaceWraps::Clone() const
{
	...
	if (face_wrap_values_ != NULL)
		clone->face_wrap_values_ = static_cast<Boolean2d*>(face_wrap_values_->Clone());
	return clone;
}
...
{
	face_wrap_values_count_ = get_next_dword(info);
	face_wrap_values_ = static_cast<Boolean2d*>(instanciate_template_by_name(info, UNICODE_STRING("Boolean2d")));
