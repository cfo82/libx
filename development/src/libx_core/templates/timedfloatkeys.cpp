...
Template* TimedFloatKeys::Clone() const
{
	...
	clone->time_ = time_;
	if (keys_ != NULL)
		clone->keys_ = static_cast<FloatKeys*>(keys_->Clone());
	return clone;
}
...
)
{
	...
	time_ = get_next_dword(info);
	keys_ = static_cast<FloatKeys*>(instanciate_template_by_name(info, UNICODE_STRING("FloatKeys")));
