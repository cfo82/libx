		...
		case Token::TypeCString:
			{
				...
				TokenName* name_token = static_cast<TokenName*>(info.scanner->ReadOptionalToken(info, Token::TypeName));
				if (name_token)
		...
		case Token::TypeArray:
			{
				...
				{
					TokenName* name_token = static_cast<TokenName*>(token);
					element->is_array_primitive_type = false;
					element->array_name_type = name_token->GetName();
				}

				// name 
				TokenName* array_name_token = static_cast<TokenName*>(info.scanner->ReadExpectedToken(info, Token::TypeName));
				if (!array_name_token)
				{
					// TODO: ERROR CASE
				}
				...
				while (do_continue)
				{
					...
					else
					{
						...
						{
							TokenInteger* integer_token = static_cast<TokenInteger*>(token);
						...
						else
						{
							...
							info.error_message += UNICODE_STRING("Token::TypeInteger or Token::TypeName expected!");
							return;
						}
		...
		case Token::TypeName:
			{
				...
				TokenName* ref_type_token = static_cast<TokenName*>(token);
...
)
{
	...
	case Token::TypeUnicode:
	case Token::TypeCString: {
		TokenString* string_token = static_cast<TokenString*>(info.scanner->ReadExpectedToken(info, Token::TypeString));
		if (!string_token)
		{
			// TODO: ERROR CASE
		}
		values_.push_back(new XGenericStringValue(string_token->GetValue()));
		break;
	}
	default:
		info.error_flag = true;
		info.error_message += UNICODE_STRING("internal error, invalid element!\n");
		return;
	}

	const string_t& name = element->primitive_name;
	values_map_.insert(pair<string_t, uint_t>(name, (uint_t)values_.size() - 1));
}
...
{
	info.error_flag = true;
	info.error_message += UNICODE_STRING("todo!\n");
}
