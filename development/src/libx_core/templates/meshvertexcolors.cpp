...
Template* MeshVertexColors::Clone() const
{
	...
	{
		clone->vertex_colors_.push_back(static_cast<IndexedColor*>((*begin)->Clone()));
	}
...
)
{
	 ...
	 */
	vertex_colors_.clear();
	vertex_colors_count_ = get_next_dword(info);
	for (udword_t i = 0; i < vertex_colors_count_; ++i)
	{
		IndexedColor* indexed_color = static_cast<IndexedColor*>(instanciate_template_by_name(info, UNICODE_STRING("IndexedColor")));
