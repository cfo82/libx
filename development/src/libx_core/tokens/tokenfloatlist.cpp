...
void TokenFloatList::SetValue(TemporalInfo& info, const float_t value)
{
	...
	else
	{
		...
		info.error_message += UNICODE_STRING("32bit-function called while parsing 64bit-file!");
	}
}
...
)
{
	...
	{
		info.error_flag = true;
		info.error_message += UNICODE_STRING("64bit-function called while parsing 32bit-file!");
	}
	else
	{
