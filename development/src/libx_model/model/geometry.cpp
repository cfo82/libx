...
//-----------------------------------------------------------------------------------------------------------
Vector4Stream* Geometry::CreateDefaultVector4Stream() const
{
	return new Vector4Stream();
}
...
//-----------------------------------------------------------------------------------------------------------
Stream* Geometry::AddVector4Stream(
	const SemanticTypes semantics,
	const uint_t semantics_info,
	const uint_t element_count,
	math::Vector4f* data
...
{
	Vector4Stream* stream = CreateDefaultVector4Stream();
	stream->SetSemantics(semantics);
	stream->SetSemanticsInfo(semantics_info);
	stream->SetData(element_count, data);
	AddStream(stream);
...
)
{
	...
	if (bone_indices_stream == NULL && bone_weights == NULL)
	{
		return;
	}
	...
	case ValueVector4: bone_count = 4; break;
	}
	...
	for (uint_t i = 0; i < vertex_count_; ++i)
	{
		...
		case 4: {
			const math::Vector4f& v = bone_weights->GetVector4(i);
			weights[0] = v.x;
			weights[1] = v.y;
			weights[2] = v.z;
			weights[3] = v.w;
