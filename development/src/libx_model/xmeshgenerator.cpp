...
//-----------------------------------------------------------------------------------------------------------
/*!	\file libx_model/xmeshgenerator.cpp
...
)
{
	...
	if( (1.0f - cosom) > 0.0001f) // 0.0001 -> some epsillon
	{
		...
		sinom = sin( omega);
		sclp  = sin( (1.0f - (1-weight_frame0)) * omega) / sinom;
		sclq  = sin( (1-weight_frame0) * omega) / sinom;
	}
	else
	{
		// Very close, do linear interp (because it's faster)
		sclp = 1.0f - (1-weight_frame0);
		sclq = (1-weight_frame0);
	}

	math::Quaternionf out;
...
)
{
	...
	else
	{
		...
		float_t weight_frame0 = 1.0f - (((float_t)frame_time - (float_t)frame0_time) / time_diff);
		float_t weight_frame1 = 1.0f - weight_frame0;
		assert(weight_frame0 >= 0.0f && weight_frame0 <= 1.0f);
		assert(weight_frame1 >= 0.0f && weight_frame1 <= 1.0f);
...
//-----------------------------------------------------------------------------------------------------------
math::Quaternionf interpolate_frames(
	const dword_t frame_time,
	const dword_t frame0_time, const math::Quaternionf& rotation0,
	const dword_t frame1_time, const math::Quaternionf& rotation1
)
{
	if (frame0_time == frame1_time)
	{
	...
	else
	{
		...
		return slerp_no_invert(rotation0, rotation1, weight_frame0);
	}
}
...
//-----------------------------------------------------------------------------------------------------------
math::Vector3f interpolate_frames(
	const dword_t frame_time,
	const dword_t frame0_time, const math::Vector3f& position0,
	const dword_t frame1_time, const math::Vector3f& position1
)
{
	if (frame0_time == frame1_time)
	{
	...
	else
	{
		...
		return interpolate(position0, position1, weight_frame0, weight_frame1);
	}
}
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeAnimationSet: {
			...
			CollectFramesReferencedFromAnimation(static_cast<x::templates::AnimationSet*>(x_template), referenced_frames);
			...
			animation->SetName(static_cast<x::templates::AnimationSet*>(x_template)->GetName());
			animation->SetFramesPerSecond(0);
			animation->SetFrameCount(0);
			animation_group->AddAnimation(animation);

			ApplyAnimationSetTemplate(static_cast<x::templates::AnimationSet*>(x_template), mesh->GetSkeleton(), animation);
			break;
		}
		}
	}
	...
	)
	{
		...
		case x::templates::TemplateTypeFrame: {
			...
			this->ApplyFrameTemplate(static_cast<x::templates::Frame*>(x_template), frame);
			break;
		}
		case x::templates::TemplateTypeMesh: {
			model::Frame* frame = mesh->GetRootFrame();
			this->ApplyMeshTemplate(static_cast<x::templates::Mesh*>(x_template), frame);
			break;
		}
		}
	}
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeFrameTransformMatrix:
			this->ApplyFrameTransformMatrix(static_cast<x::templates::FrameTransformMatrix*>(x_template), frame);
			break;
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeMeshTextureCoords: {
			this->ApplyMeshTextureCoords(static_cast<x::templates::MeshTextureCoords*>(x_template), entity, texture_unit);
			++texture_unit;
			break;
		}
		...
		case x::templates::TemplateTypeSkinWeights: {
			skinweights.push_back(static_cast<x::templates::SkinWeights*>(x_template));
			break;
		}
		}
	}
	...
	)
	{
		...
		{
			this->ApplyMeshNormalsTemplate(static_cast<x::templates::MeshNormals*>(x_template), entity);
		}
	}
	...
	)
	{
		...
		{
			this->ApplyMeshMaterialListTemplate(static_cast<x::templates::MeshMaterialList*>(x_template), parent_frame, entity);
		}
	}
}
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeTextureFilename:
			this->ApplyTextureFilenameTemplate(static_cast<x::templates::TextureFilename*>(x_template), material);
			break;
		}
	}
...
)
{
	...
	ubyte_t* current_bone_per_vertex_array = new ubyte_t[vertex_count];
	memset(current_bone_per_vertex_array, 0, sizeof(ubyte_t) * vertex_count);

	// collect count of bones / vertex
	for (uint_t i = 0; i < skinweights.size(); ++i)
	{
		x::templates::SkinWeights* weights = skinweights[i];
		const udword_t* vertex_indices = weights->GetVertexIndices();
		const float_t* vertex_weights = weights->GetWeights();
		for (udword_t j = 0; j < weights->GetNumWeights(); ++j)
		{
			if (vertex_weights[j] > 0.0f)
			{
	...
	case 4:
	case 5: {
		math::Vector4f* bone_weights_data = new math::Vector4f[vertex_count];
		memset(bone_weights_data, 0, sizeof(math::Vector4f) * vertex_count);
		bone_weight_stream = entity->GetGeometry()->AddVector4Stream(model::SemanticTypeBoneWeights, 0, vertex_count, bone_weights_data);
		break;
	}
	default: {
		throw Exception("given number of bones/vertex is not supported!");
	...
	for (uint_t i = 0; i < skinweights.size(); ++i)
	{
		...
		for (dword_t j = 0; j < num_weights; ++j)
		{
			...
			if (current_bone_per_vertex_array[vertex_index] < 4)
			{
				bone_indices[current_bone_per_vertex_array[vertex_index]] = bone_index;
			}
			...
			case 0:
			case 1: {
				break; // this led to error earlier
			}
			case 2: {
				math::Vector2f v = bone_weight_stream->GetVector2(vertex_index);
				if (current_bone_per_vertex_array[vertex_index] < 2)
				{
					v[current_bone_per_vertex_array[vertex_index]] = vertex_weights[j];
				}
			...
			case 3: {
				...
				if (current_bone_per_vertex_array[vertex_index] < 3)
				{
					v[current_bone_per_vertex_array[vertex_index]] = vertex_weights[j];
				}
			...
			default: {
				math::Vector4f v = bone_weight_stream->GetVector4(vertex_index);
				if (current_bone_per_vertex_array[vertex_index] < 4)
				{
					v[current_bone_per_vertex_array[vertex_index]] = vertex_weights[j];
				}
				...
				{
					bool_t changed = true;
					float_t values[] = { v.x, v.y, v.z, v.w, vertex_weights[j] };
					while (changed)
					{
						changed = false;
					...
					if (values[0] != vertex_weights[j])
					{
						// replace the smallest value (value[0]) and renormalize the weights then
						float_t sum = values[1] + values[2] + values[3] + values[4];
						math::Vector4<ubyte_t> bone_indices = bone_index_stream->GetUByte4(vertex_index);
						for (uint_t i = 0; i < 4; ++i)
						{
							if (v[i] == values[0])
							{
								v[i] = vertex_weights[j];
				...
				bone_weight_stream->SetVector4(vertex_index, v);
				break;
			}
			}

			// increment index for this vertex's bone
			++current_bone_per_vertex_array[vertex_index];
		}
	}
	...
	delete[] current_bone_per_vertex_array;
}
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeFrame: {
			LoadSkeleton(mesh, static_cast<x::templates::Frame*>(x_template), referenced_frames);
			break;
		}
		}
	}
...
//-----------------------------------------------------------------------------------------------------------
bool_t XMeshGenerator::LoadSkeleton(
	model::Model* mesh,
	x::templates::Frame* x_frame,
	vector<string_t>& referenced_frames
)
{
	...
	{
		x::templates::Template* x_template = (*begin);
		if (x_template->GetType() == x::templates::TemplateTypeFrame)
		{
			if (LoadSkeleton(mesh, static_cast<x::templates::Frame*>(x_template), referenced_frames))
			{
				return true;
			}
		}
...
)
{
	...
	)
	{
		...
		{
		case x::templates::TemplateTypeFrameTransformMatrix: {
			this->ApplyBoneTransformMatrix(static_cast<x::templates::FrameTransformMatrix*>(x_template), new_bone);
			break;
		}
		case x::templates::TemplateTypeFrame: {
			this->LoadBone(static_cast<x::templates::Frame*>(x_template), skeleton, new_bone);
			break;
		}
		}
	}
...
)
{
	...
	)
	{
		...
		case x::templates::TemplateTypeAnimationKey: {
			GetMinDiffAndLastFrameTime(static_cast<x::templates::AnimationKey*>(x_template), min_diff, last_frame_time);
			break;
		}
		}
	}
...
)
{
	...
	)
	{
		...
		{
		case x::templates::TemplateTypeAnimationKey: {
			ApplyAnimationKeyTemplate(static_cast<x::templates::AnimationKey*>(x_template), x_animation->GetReference(), skeleton, animation, frame_diff, last_frame_time);
			break;
		}
		}
	}
...
)
{
	...
	{
	case 0: {
		ApplyRotationAnimationKeyTemplate(x_animation_key, referenced_bone, skeleton, animation, frame_diff, last_frame_time);
		break;
	}
	case 1: {
		ApplyScaleAnimationKeyTemplate(x_animation_key, referenced_bone, skeleton, animation, frame_diff, last_frame_time);
		break;
	}
	case 2: {
		ApplyPositionAnimationKeyTemplate(x_animation_key, referenced_bone, skeleton, animation, frame_diff, last_frame_time);
		break;
	}
	case 4: {
...
//-----------------------------------------------------------------------------------------------------------
void XMeshGenerator::ApplyRotationAnimationKeyTemplate(
	x::templates::AnimationKey* x_animation_key,
	const string_t& referenced_bone,
	model::Skeleton* skeleton,
	model::Animation* animation,
...
)
{
	...
	for (uint_t i = 0; i < frame_count; ++i)
	{
		...
		if (float_keys0->GetNumValues() != 4)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values0 = float_keys0->GetValues();
		math::Quaternionf rotation0(values0[0], -values0[1], -values0[2], -values0[3]);
		// get frame1 data
		const x::templates::TimedFloatKeys* key1 = keys[frame1];
		const x::templates::FloatKeys* float_keys1 = key1->GetKeys();
		if (float_keys1->GetNumValues() != 4)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values1 = float_keys1->GetValues();
		math::Quaternionf rotation1(values1[0], -values1[1], -values1[2], -values1[3]);

		// interpolate and save data
		rotations[i] = interpolate_frames(
			frame_time,
			key0->GetTime(), rotation0,
			key1->GetTime(), rotation1
		);
	}
	...
	rotate_track->SetAnimatedQuaternion(bone, model::Bone::ChannelRotation);
...
//-----------------------------------------------------------------------------------------------------------
void XMeshGenerator::ApplyScaleAnimationKeyTemplate(
	x::templates::AnimationKey* x_animation_key,
	const string_t& referenced_bone,
	model::Skeleton* skeleton,
	model::Animation* animation,
...
)
{
	...
	for (uint_t i = 0; i < frame_count; ++i)
	{
		...
		const x::templates::TimedFloatKeys* key0 = keys[frame0];
		const x::templates::FloatKeys* float_keys0 = key0->GetKeys();
		if (float_keys0->GetNumValues() != 3)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values0 = float_keys0->GetValues();
		math::Vector3f scale0(values0[0], values0[1], values0[2]);
		// get frame1 data
		const x::templates::TimedFloatKeys* key1 = keys[frame1];
		const x::templates::FloatKeys* float_keys1 = key1->GetKeys();
		if (float_keys1->GetNumValues() != 3)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values1 = float_keys1->GetValues();
		math::Vector3f scale1(values1[0], values1[1], values1[2]);

		// interpolate and save data
		scales[i] = interpolate_frames(
			frame_time,
			key0->GetTime(), scale0,
			key1->GetTime(), scale1
		);
	...
	// create rotation track for this bone
	model::VectorAnimationTrack* scale_track = animation->CreateVectorAnimationTrack();
	scale_track->SetAnimatedVector(bone, model::Bone::ChannelScale);
	scale_track->SetKeyFrames(scales);
	animation->AddAnimationTrack(scale_track);
...
//-----------------------------------------------------------------------------------------------------------
void XMeshGenerator::ApplyPositionAnimationKeyTemplate(
	x::templates::AnimationKey* x_animation_key,
	const string_t& referenced_bone,
	model::Skeleton* skeleton,
	model::Animation* animation,
...
)
{
	...
	for (uint_t i = 0; i < frame_count; ++i)
	{
		...
		const float_t* values0 = float_keys0->GetValues();
		math::Vector3f translation0(values0[0], values0[1], values0[2]);
		// get frame1 data
		const x::templates::TimedFloatKeys* key1 = keys[frame1];
		const x::templates::FloatKeys* float_keys1 = key1->GetKeys();
		if (float_keys1->GetNumValues() != 3)
		...
		const float_t* values1 = float_keys1->GetValues();
		math::Vector3f translation1(values1[0], values1[1], values1[2]);

		// interpolate and save data
		translations[i] = interpolate_frames(
			frame_time,
			key0->GetTime(), translation0,
			key1->GetTime(), translation1
		);
	}
	...
	translate_track->SetAnimatedVector(bone, model::Bone::ChannelTranslation);
...
)
{
	...
	for (uint_t i = 0; i < frame_count; ++i)
	{
		...
		const x::templates::TimedFloatKeys* key0 = keys[frame0];
		const x::templates::FloatKeys* float_keys0 = key0->GetKeys();
		if (float_keys0->GetNumValues() != 16)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values0 = float_keys0->GetValues();
		...
		const x::templates::TimedFloatKeys* key1 = keys[frame1];
		const x::templates::FloatKeys* float_keys1 = key1->GetKeys();
		if (float_keys1->GetNumValues() != 16)
		{
			throw Exception(UNICODE_STRING("invalid number of values inside an animation key"));
		}
		const float_t* values1 = float_keys1->GetValues();
	...
	rotate_track->SetKeyFrames(rotations);
	animation->AddAnimationTrack(rotate_track);
	// create scale track for this bone
	model::VectorAnimationTrack* scale_track = animation->CreateVectorAnimationTrack();
	scale_track->SetAnimatedVector(bone, model::Bone::ChannelScale);
	scale_track->SetKeyFrames(scales);
	animation->AddAnimationTrack(scale_track);
...
)
{
	...
	if (grow_size > 0)
	{
		...
		for (uint_t i = 0; i < entity->GetGeometry()->GetStreamCount(); ++i)
		{
			...
			case model::ValueVector4:{
				math::Vector4f* new_data(new math::Vector4f[new_vertex_count]);
				model::Vector4Stream* new_stream_inst = entity->GetGeometry()->CreateDefaultVector4Stream();
				new_stream_inst->SetData(new_vertex_count, new_data);
				for (uint_t j = 0; j < stream->GetElementCount(); ++j)
					new_stream_inst->SetVector4(j, stream->GetVector4(j));
				new_stream_inst->SetSemantics(stream->GetSemantics());
				new_stream_inst->SetSemanticsInfo(stream->GetSemanticInfo());
				new_stream = new_stream_inst;
				break;
		...
		for (uint_t i = 0; i < face_count*3; ++i)
		{
			...
			if (ref_count[old_index] > 1)
			{
				...
				for (uint_t j = 0; j < entity->GetGeometry()->GetStreamCount(); ++j)
				{
					...
					case model::ValueVector4:{
						new_stream->SetVector4(new_index, old_stream->GetVector4(old_index));
						break;
					}
					case model::ValueQuaternion: {
